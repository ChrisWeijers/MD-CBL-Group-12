...
train_data = pivoted_data[pivoted_data['Year'] <= 2023].copy()
test_data = pivoted_data[pivoted_data['Year'] >= 2024].copy()

# Prepare features (X) and target (y) for training and testing
X_train = train_data.drop(columns=['Year', 'Month', 'LSOA code'])
y_train = train_data['Burglary']
X_test = test_data.drop(columns=['Year', 'Month', 'LSOA code'])
